...
import {
  ...
  FileText, X, ZoomIn, ZoomOut, ExternalLink, Cpu, Globe,
  Code2, Zap, GraduationCap, Briefcase, MapPin, ChevronRight,
  Phone, Layers, Star, Calendar, Users, Lightbulb,
} from "lucide-react";
...
const CV = {
  ...
  skills: [
    ...
    "Node JS (Express)", "MongoDB (Mongoose)", "Python", "C/C++",
    "Git/GitHub", "Arduino", "ESP", "Raspberry Pi",
  ],
  languages: ["English", "Nepali"],
};

/* ─── RESUME DOCUMENT (print-safe, no Tailwind) ──────────── */
const ResumeDocument = ({ zoom }) => (
  <div style={{
    ...
    {/* Header */}
    <div style={{ background: "linear-gradient(135deg, #111827 0%, #1f2937 100%)", color: "#fff", padding: "32px 40px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, margin: 0, letterSpacing: -0.5 }}>{CV.name}</h1>
          <p style={{ margin: "6px 0 0", fontSize: 13, color: "#9ca3af", fontWeight: 500 }}>{CV.title}</p>
        </div>
        <div style={{ fontSize: 11, color: "#6b7280", textAlign: "right", lineHeight: 1.8 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "220px 1fr" }}>
      {/* Left sidebar */}
      <div style={{ background: "#f9fafb", borderRight: "1px solid #f3f4f6", padding: "28px 20px", display: "flex", flexDirection: "column", gap: 24 }}>
        {/* Skills */}
        <div>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 10 }}>Core Skills</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {CV.skills.map((s) => (
              <span key={s} style={{ padding: "3px 8px", background: "#e5e7eb", borderRadius: 20, fontSize: 10, fontWeight: 500, color: "#374151" }}>{s}</span>
            ))}
          </div>
        </div>
        {/* Education */}
        <div>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 10 }}>Education</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {CV.education.map((e) => (
              <div key={e.degree} style={{ borderLeft: "2px solid #d1d5db", paddingLeft: 10 }}>
                <p style={{ fontSize: 10.5, fontWeight: 700, color: "#111827", margin: 0, lineHeight: 1.4 }}>{e.degree}</p>
                <p style={{ fontSize: 9.5, color: "#6b7280", margin: "2px 0 0" }}>{e.institution}</p>
                <p style={{ fontSize: 9, color: "#9ca3af", margin: "2px 0 0" }}>{e.period}</p>
              </div>
            ))}
          </div>
        </div>
        {/* Languages */}
        <div>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 10 }}>Languages</p>
          {CV.languages.map((l) => (
      ...
      <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 22 }}>
        {/* Summary */}
        <div>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 8 }}>Profile</p>
          <p style={{ fontSize: 11, lineHeight: 1.7, color: "#4b5563", margin: 0 }}>{CV.summary}</p>
        </div>
        {/* Experience */}
        <div>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 10 }}>Experience</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {CV.experience.map((exp) => (
              <div key={exp.role} style={{ borderLeft: "2px solid #111827", paddingLeft: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                  <div>
                    <p style={{ fontSize: 12, fontWeight: 700, color: "#111827", margin: 0 }}>{exp.role}</p>
                    <p style={{ fontSize: 10, color: "#6b7280", margin: "2px 0 0", fontWeight: 500 }}>{exp.company} · {exp.location}</p>
                  </div>
                  <span style={{ fontSize: 9, color: "#9ca3af", whiteSpace: "nowrap", marginTop: 2 }}>{exp.period}</span>
                </div>
                <ul style={{ margin: "8px 0 0", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 4 }}>
                  {exp.bullets.map((b, i) => (
                    <li key={i} style={{ display: "flex", gap: 6, fontSize: 10, color: "#4b5563", lineHeight: 1.5 }}>
                      <span style={{ color: "#9ca3af", marginTop: 1 }}>›</span>{b}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {/* Projects */}
        <div>
          <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 10 }}>Projects</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {CV.projects.map((p) => (
              <div key={p.name} style={{ borderLeft: "2px solid #d1d5db", paddingLeft: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#111827", margin: 0 }}>{p.name}</p>
                </div>
                <p style={{ fontSize: 9, color: "#9ca3af", fontFamily: "monospace", margin: "2px 0 6px" }}>{p.stack}</p>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 6 : 8 }}>
          ...
          <div style={{
            ...
            <button onClick={() => setZoom((z) => Math.max(z - 0.1, 0.3))}
              style={{ padding: isMobile ? "6px 8px" : "7px 10px", background: "transparent", border: "none", cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center" }}>
              <ZoomOut size={13} />
            </button>
            <span style={{ fontSize: 11, fontFamily: "monospace", color: "#64748b", minWidth: isMobile ? 32 : 40, textAlign: "center" }}>
              {Math.round(zoom * 100)}%
            </span>
            <button onClick={() => setZoom((z) => Math.min(z + 0.1, 1.4))}
              style={{ padding: isMobile ? "6px 8px" : "7px 10px", background: "transparent", border: "none", cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center" }}>
              <ZoomIn size={13} />
            </button>
          </div>
...
const projects = [
  {
    ...
    description: "YOLOv8 + FaceNet AI face recognition with real-time tracking, MongoDB, and automated email alerts.",
    stack: ["YOLOv8", "FaceNet", "React", "Node.js", "MongoDB"],
    url: "https://github.com/Aashik9567/Automated_Attendance_Management_System",
  ...
  },
  {
    ...
    description: "Full-stack system with role-based dashboards for Admin, Teacher, Student & Parent. AI analytics built in.",
    stack: ["React", "Node.js", "Express", "MongoDB"],
    url: "https://github.com/Aashik9567/Smart-School-management-system-Frontend",
  ...
  {
    name: "Cable Network Website",
    description: "Production ISP website with 3D React Three Fiber visuals, TypeScript, Ant Design, and theme switching.",
  ...
  },
  {
    ...
    description: "Real-time GPS weather data, AI-powered analysis, 10-day forecast, and dark glassmorphism UI.",
...
const HomePage = () => {
  ...
  const hardwareSkills = ["Arduino", "ESP32", "Raspberry Pi", "Python", "C/C++", "IoT Protocols", "Circuit Design"];
  const webSkills = ["React", "Next.js", "TypeScript", "Node.js", "MongoDB", "Tailwind CSS", "Express"];
    ...
    <div className="relative overflow-hidden">
              ...
              <div className="order-2 lg:order-1">
                ...
                <div className="mb-2">
                  <SplitText text="Hi, I'm" tag="h1"
                    ...
                    delay={40} splitType="chars" />
                </div>
                <div className="mb-6">
                  <SplitText text="Aashiq Mahato" tag="h1"
                ...
                <ScrollReveal delay={0.3}>
                  <div className="flex items-center h-10 gap-3 mb-8">
                    ...
                    <RotatingText words={roles} interval={3000}
                ...
                <ScrollReveal delay={0.4}>
                  <p className="max-w-2xl mb-10 text-lg leading-relaxed text-muted-foreground">
                    Electronics Engineer &amp; Full-Stack Developer bridging the physical world with
                    scalable digital ecosystems. Specializing in IoT, embedded systems, and modern web technologies.
                  </p>
                </ScrollReveal>
              ...
              <ScrollReveal direction="right" delay={0.3} className="flex justify-center order-1 lg:order-2">
                <Magnet strength={0.1}>
                  <div className="relative">
                    ...
                    {/* floating chips */}
                    <motion.div animate={shouldReduceMotion ? undefined : { y: [0, -8, 0] }}
                      transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                      className="absolute -top-4 -left-4 z-20 px-3 py-1.5 rounded-full text-xs font-bold shadow-lg border border-blue-500/30 bg-blue-500/10 text-blue-300 backdrop-blur">
                      ⚡ IoT Engineer
                    </motion.div>
                    <motion.div animate={shouldReduceMotion ? undefined : { y: [0, 8, 0] }}
                      transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
                      className="absolute -bottom-4 -right-4 z-20 px-3 py-1.5 rounded-full text-xs font-bold shadow-lg border border-purple-500/30 bg-purple-500/10 text-purple-300 backdrop-blur">
                      🚀 Full-Stack Dev
                    </motion.div>
                    <motion.div animate={shouldReduceMotion ? undefined : { y: [0, -5, 0] }}
                      transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                      className="absolute top-1/2 -right-8 z-20 px-3 py-1.5 rounded-full text-xs font-bold shadow-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 backdrop-blur">
                      🛠 Arduino
                    </motion.div>
                    ...
                    <div className="relative overflow-hidden border shadow-sm w-72 h-72 rounded-3xl sm:w-80 sm:h-80 lg:w-96 lg:h-96 border-border bg-card">
                      <img src={profileImage} alt="Aashiq Mahato" className="object-cover w-full h-full" loading="eager" decoding="async" />
                    </div>
                  </div>
                </Magnet>
              </ScrollReveal>
        ...
        <div className="mx-auto max-w-7xl">
          <ScrollReveal>
            <div className="text-center mb-14">
              <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-widest border border-border bg-card/60 text-muted-foreground mb-4">Core Expertise</span>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <ScrollReveal direction="left" delay={0.1}>
              <motion.div whileHover={{ y: -4 }} className="relative h-full p-8 overflow-hidden border cursor-default group rounded-2xl border-orange-500/20 bg-gradient-to-br from-orange-500/10 to-red-500/5">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="mb-2 text-xl font-bold font-display text-foreground">Hardware Engineer</h3>
                  <p className="mb-6 text-sm leading-relaxed text-muted-foreground">
                    Designing smart hardware prototypes and IoT systems that interact with the physical world — from sensor networks to embedded firmware.
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {hardwareSkills.map((s) => (
                      <span key={s} className="px-3 py-1 text-xs font-semibold text-orange-300 border rounded-lg bg-orange-500/15 border-orange-500/25">{s}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
            ...
            <ScrollReveal direction="right" delay={0.2}>
              <motion.div whileHover={{ y: -4 }} className="relative h-full p-8 overflow-hidden border cursor-default group rounded-2xl border-primary/20 bg-gradient-to-br from-primary/10 to-secondary/5">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="mb-2 text-xl font-bold font-display text-foreground">Full-Stack Developer</h3>
                  <p className="mb-6 text-sm leading-relaxed text-muted-foreground">
                    Building performance-optimized, interactive web architectures — from blazing-fast Next.js frontends to robust Express APIs.
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {webSkills.map((s) => (
                      <span key={s} className="px-3 py-1 text-xs font-semibold border rounded-lg bg-primary/15 border-primary/25 text-primary-300">{s}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
      ...
      <section className="py-24 section-padding bg-card/20">
        <div className="mx-auto max-w-7xl">
          <ScrollReveal>
            <div className="text-center mb-14">
              <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-widest border border-border bg-card/60 text-muted-foreground mb-4">Featured Work</span>
              <h2 className="text-3xl font-bold font-display text-foreground">
              ...
              return (
                <ScrollReveal key={p.name} delay={i * 0.1}>
                  <motion.a href={p.url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="absolute w-24 h-24 transition-opacity duration-500 rounded-full -top-6 -right-6 opacity-20 group-hover:opacity-40 blur"
                      ...
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full border text-muted-foreground"
                            style={{ borderColor: `${p.accent}33`, background: `${p.accent}11` }}>
                            {p.tag}
                          </span>
                          <ExternalLink size={14} className="transition-colors text-muted-foreground group-hover:text-foreground" />
                        </div>
                      </div>
                      <h3 className="mb-2 text-base font-bold text-foreground font-display">{p.name}</h3>
                      <p className="flex-1 mb-5 text-sm leading-relaxed text-muted-foreground">{p.description}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {p.stack.map((s) => (
                          <span key={s} className="px-2 py-0.5 text-[10px] font-mono font-medium rounded-md bg-background/40 border border-border text-muted-foreground">{s}</span>
                        ))}
                      </div>
                    </div>
                  </motion.a>
      ...
      <section className="py-24 section-padding">
        <div className="max-w-4xl mx-auto">
          <ScrollReveal>
            <div className="text-center mb-14">
              <span className="inline-block px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-widest border border-border bg-card/60 text-muted-foreground mb-4">Journey</span>
              <h2 className="text-3xl font-bold font-display text-foreground">
                ...
                return (
                  <ScrollReveal key={item.title} delay={i * 0.12}>
                    <motion.div whileHover={{ x: 4 }} className="relative group">
                      {/* dot */}
                      <div className="absolute -left-[52px] top-1 w-8 h-8 rounded-full flex items-center justify-center border-2 border-background shadow-sm transition-transform duration-300 group-hover:scale-110"
                      ...
                      <div className="p-5 transition-colors border rounded-2xl border-border bg-card/60 backdrop-blur hover:bg-card/80">
                        <div className="flex flex-wrap items-start justify-between gap-4 mb-1">
                          ...
                          <span className="text-xs font-mono text-muted-foreground px-2 py-0.5 rounded-full bg-muted/50 border border-border whitespace-nowrap">{item.date}</span>
                        </div>
                        <p className="mb-2 text-xs font-semibold" style={{ color: item.color }}>{item.subtitle}</p>
                        <p className="text-sm leading-relaxed text-muted-foreground">{item.desc}</p>
                        <div className="mt-2">
                          <span className={`text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full ${item.type === "work" ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-violet-500/10 text-violet-400 border border-violet-500/20"}`}>
