...
import {
  Search, Home, User, Code2, FolderKanban, GraduationCap, Mail,
  LayoutDashboard, BookOpen, Compass, ExternalLink, Github, Download,
  Terminal, Moon, Sun, Monitor, Briefcase, Sparkles, ArrowRight,
  Clock, Star, Cpu, FileText, ChevronRight, BarChart2
} from 'lucide-react';
...
const buildCommands = (navigate, theme, setTheme, isRecruiterMode, toggleRecruiterMode) => [
  // Navigation
  {
    group: 'Navigation',
    id: 'nav-home',
  ...
  },
  {
    ...
    description: isRecruiterMode ? 'Return to full portfolio' : 'Simplified view for recruiters',
    icon: Briefcase,
  ...
  },
  {
    ...
    description: 'github.com/Aashik9567',
    icon: Github,
    keywords: ['github', 'profile', 'code', 'repos'],
    action: () => window.open('https://github.com/Aashik9567', '_blank'),
  },
  // Projects
  ...projects.map(p => ({
    group: 'Projects',
    ...
    icon: Star,
    keywords: [p.slug, ...p.tags.map(t => t.toLowerCase()), p.category?.toLowerCase()],
...
const CommandPalette = () => {
  ...
  const allCommands = useMemo(
    () => buildCommands(navigate, theme, setTheme, isRecruiterMode, toggleRecruiterMode)
      .filter(c => !c.hidden),
  ...
  // Group icons
  const groupIcon = { Navigation: ArrowRight, Projects: Star, Blog: BookOpen, Actions: Sparkles };
                          ...
                          return (
                            ...
                            >
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
                                isSelected ? 'bg-primary/15 text-primary' : 'bg-muted/40'
                              }`}>
