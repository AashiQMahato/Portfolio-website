...
import {
  MapPin, Calendar, Mail, Phone, Award, Users, Lightbulb,
  BookOpen, Cpu, Globe, Zap, Code2, GraduationCap, Layers,
  ArrowRight, Sparkles, ZoomIn, ZoomOut, Download, X, FileText,
  Github, Linkedin, ExternalLink, Briefcase, ChevronRight,
} from "lucide-react";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ fontSize: isMobile ? 20 : 28, fontWeight: 800, margin: 0, letterSpacing: -0.5 }}>{CV.name}</h1>
            <p style={{ margin: "5px 0 0", fontSize: isMobile ? 11 : 13, color: "#9ca3af", fontWeight: 500 }}>{CV.title}</p>
          </div>
          <div style={{ fontSize: isMobile ? 10 : 11, color: "#6b7280", lineHeight: 1.8, textAlign: isMobile ? "left" : "right" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: isMobile ? "1 1 100%" : "unset" }}>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 8 }}>Core Skills</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
              {CV.skills.map((s) => (
                <span key={s} style={{ padding: "2px 7px", background: "#e5e7eb", borderRadius: 20, fontSize: 9.5, fontWeight: 500, color: "#374151" }}>{s}</span>
              ))}
            </div>
          </div>
          {/* Education */}
          <div style={{ flex: isMobile ? "1 1 45%" : "unset" }}>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 8 }}>Education</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {CV.education.map((e) => (
                <div key={e.degree} style={{ borderLeft: "2px solid #d1d5db", paddingLeft: 8 }}>
                  <p style={{ fontSize: 10, fontWeight: 700, color: "#111827", margin: 0, lineHeight: 1.4 }}>{e.degree}</p>
                  <p style={{ fontSize: 9, color: "#6b7280", margin: "2px 0 0" }}>{e.institution}</p>
                  <p style={{ fontSize: 8.5, color: "#9ca3af", margin: "2px 0 0" }}>{e.period}</p>
                </div>
              ))}
            </div>
          </div>
          {/* Languages */}
          <div style={{ flex: isMobile ? "1 1 45%" : "unset" }}>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 8 }}>Languages</p>
            {CV.languages.map((l) => (
        ...
        {/* Main */}
        <div style={{ padding: isMobile ? "18px 18px" : "28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
          {/* Summary */}
          <div>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 6 }}>Profile</p>
            <p style={{ fontSize: isMobile ? 10.5 : 11, lineHeight: 1.7, color: "#4b5563", margin: 0 }}>{CV.summary}</p>
          </div>
          {/* Experience */}
          <div>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 8 }}>Experience</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {CV.experience.map((exp) => (
                <div key={exp.role} style={{ borderLeft: "2px solid #111827", paddingLeft: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, flexWrap: "wrap" }}>
                    <div>
                      <p style={{ fontSize: isMobile ? 11 : 12, fontWeight: 700, color: "#111827", margin: 0 }}>{exp.role}</p>
                      <p style={{ fontSize: 9.5, color: "#6b7280", margin: "2px 0 0", fontWeight: 500 }}>{exp.company} · {exp.location}</p>
                    </div>
                    <span style={{ fontSize: 9, color: "#9ca3af", whiteSpace: "nowrap" }}>{exp.period}</span>
                  </div>
                  <ul style={{ margin: "6px 0 0", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 3 }}>
                    {exp.bullets.map((b, i) => (
                      <li key={i} style={{ display: "flex", gap: 5, fontSize: isMobile ? 9.5 : 10, color: "#4b5563", lineHeight: 1.5 }}>
          ...
          {/* Projects */}
          <div>
            <p style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#9ca3af", marginBottom: 8 }}>Projects</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {CV.projects.map((p) => (
                <div key={p.name} style={{ borderLeft: "2px solid #d1d5db", paddingLeft: 12 }}>
                  <p style={{ fontSize: isMobile ? 10.5 : 11, fontWeight: 700, color: "#111827", margin: 0 }}>{p.name}</p>
                  <p style={{ fontSize: 9, color: "#9ca3af", fontFamily: "monospace", margin: "2px 0 5px" }}>{p.stack}</p>
    ...
    <motion.div
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
          ...
          <span style={{ fontSize: isMobile ? 12 : 14, fontWeight: 600, color: "#f1f5f9", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 6 : 8, flexWrap: "nowrap" }}>
          ...
          {!isMobile && (
            <div style={{ display: "flex", alignItems: "center", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, overflow: "hidden" }}>
              <button onClick={() => setZoom((z) => Math.max(z - 0.1, 0.4))}
                style={{ padding: "7px 10px", background: "transparent", border: "none", cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center" }}>
                <ZoomOut size={13} />
              </button>
              <span style={{ fontSize: 11, fontFamily: "monospace", color: "#64748b", minWidth: 40, textAlign: "center" }}>
                {Math.round(zoom * 100)}%
              </span>
          ...
          {isMobile && (
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <button onClick={() => setZoom((z) => Math.max(z - 0.1, 0.3))}
                style={{ padding: "6px 8px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center" }}>
                <ZoomOut size={13} />
              </button>
              <span style={{ fontSize: 10, fontFamily: "monospace", color: "#64748b", minWidth: 32, textAlign: "center" }}>
                {Math.round(zoom * 100)}%
              </span>
              <button onClick={() => setZoom((z) => Math.min(z + 0.1, 1.2))}
                style={{ padding: "6px 8px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center" }}>
                <ZoomIn size={13} />
              </button>
            </div>
...
const skillColors = [
  ...
  "from-red-500/20 to-red-600/10 border-red-500/30 text-red-300",
...
const About = () => {
  ...
  const skills = CV.skills.map((name, i) => ({
    name, color: skillColors[i % skillColors.length],
  }));

  const values = [
    {
      title: "Innovation", icon: Lightbulb,
      description: "Developing creative solutions that bridge hardware and software, turning ideas into real-world impact.",
      gradient: "from-amber-500 to-orange-500", bgGlow: "from-amber-500/10 to-orange-500/5",
      border: "border-amber-500/20", iconBg: "bg-amber-500/10", tag: "Creative Thinking",
    },
    {
      title: "Excellence", icon: Award,
      description: "Maintaining high standards in every line of code — from embedded firmware to modern web applications.",
      gradient: "from-primary-500 to-primary-600", bgGlow: "from-primary-500/10 to-primary-600/5",
      border: "border-primary-500/20", iconBg: "bg-primary-500/10", tag: "Quality First",
    },
    {
      title: "Collaboration", icon: Users,
      description: "Working effectively in cross-functional teams to deliver complex projects combining diverse expertise.",
      gradient: "from-secondary-500 to-secondary-600", bgGlow: "from-secondary-500/10 to-secondary-600/5",
      border: "border-secondary-500/20", iconBg: "bg-secondary-500/10", tag: "Team Player",
    },
    {
      title: "Learning", icon: BookOpen,
      description: "Staying current with emerging technologies and frameworks to tackle next-generation engineering challenges.",
      gradient: "from-violet-500 to-purple-600", bgGlow: "from-violet-500/10 to-purple-600/5",
      border: "border-violet-500/20", iconBg: "bg-violet-500/10", tag: "Growth Mindset",
    },
  ];
        ...
        <div className="mx-auto max-w-7xl">
              ...
              <div className="space-y-6">
                ...
                <div className="relative w-64 h-64 mx-auto lg:w-full lg:h-80">
                  ...
                  <div className="relative w-full h-full rounded-2xl overflow-hidden glass p-[2px]">
                    <img src={profileImage} alt="Aashiq Mahato" className="object-cover w-full h-full rounded-2xl" />
                    ...
                    return (
                      <motion.div key={info.label} whileHover={{ x: 4 }}
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-sm font-medium truncate text-foreground">{info.value}</p>
                        </div>
                      </motion.div>
                    );
                  })}
                  ...
                  <div className="relative z-10 space-y-5 p-7">
                    ...
                    <div className="space-y-3">
                      <p className="text-sm leading-relaxed text-muted-foreground">
                        I'm <span className="font-semibold text-foreground">Aashik Kumar Mahato</span>, an
                        Electronics, Communication, and Information Engineering graduate from{" "}
                        <span className="font-medium text-primary">Advanced College of Engineering and Management</span>,
                        Kathmandu. With a passion for bridging the physical and digital worlds, I specialize in IoT
                        systems, embedded programming, and modern web development.
                      </p>
                      <p className="text-sm leading-relaxed text-muted-foreground">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {highlights.map((h) => {
                        const Icon = h.icon;
                        return (
                          <div key={h.text} className="flex items-center gap-2 p-2.5 rounded-lg bg-muted/50 border border-border">
                            <Icon className="w-3.5 h-3.5 text-primary-400 shrink-0" />
                            <span className="text-xs leading-snug text-muted-foreground">{h.text}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              <ScrollReveal delay={0.4}>
                <div className="p-5 border shadow-sm rounded-2xl border-border bg-card/60 backdrop-blur">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-sm font-semibold tracking-wide uppercase font-display text-foreground">Tech Stack</h4>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {skills.map((skill) => (
                      <motion.span key={skill.name} whileHover={{ scale: 1.06, y: -2 }}
          ...
          <ScrollReveal delay={0.1}>
            <div className="flex items-center gap-3 mb-8">
              ...
              <span className="px-4 text-xs font-semibold tracking-widest uppercase text-muted-foreground">What Drives Me</span>
              <div className="flex-1 h-px bg-gradient-to-r from-transparent via-border/70 to-transparent" />
            </div>
          </ScrollReveal>
              ...
              return (
                <ScrollReveal key={value.title} delay={i * 0.1}>
                  <motion.div whileHover={{ y: -6 }}
                    ...
                    <div className={`absolute -top-6 -right-6 w-24 h-24 rounded-full bg-gradient-to-br ${value.gradient} opacity-10 group-hover:opacity-20 transition-opacity blur`} />
                    <div className="relative z-10">
                      ...
                      <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${value.iconBg} border ${value.border} mb-3`}>
                        <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">{value.tag}</span>
                      </div>
                      <h4 className="mb-2 text-lg font-bold font-display text-foreground">{value.title}</h4>
                      <p className="text-sm leading-relaxed text-muted-foreground">{value.description}</p>
                      <div className={`mt-5 h-0.5 w-full scale-x-0 group-hover:scale-x-100 origin-left bg-gradient-to-r ${value.gradient} rounded-full transition-transform duration-500`} />
                    </div>
                  </motion.div>
                </ScrollReveal>
              );
